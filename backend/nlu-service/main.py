...
def _try_load_encoder(name_or_path: str):
    try:
        ...
        if USE_GPU:
            mdl = mdl.to(DEVICE)
            print(f"   Encoder loaded on GPU: {name_or_path}")
        return tok, mdl, name_or_path, True
    except Exception as e:
        print(f"   Failed to load encoder {name_or_path}: {e}")
...
def _load_cls_model(path: Optional[str]):
    ...
    try:
        ...
        if USE_GPU:
            ...
            print(f"   Classification model loaded on GPU: {path}")
        return tok, model, id2label
    except Exception as e:
        # Model not available locally or invalid; do not crash service
        print(f"   Failed to load classification model {path}: {e}")
        return None, None, None
...
def _load_tokcls_model(path: Optional[str]):
    ...
    try:
        ...
        if USE_GPU:
            model = model.to(DEVICE)
            print(f"   Token classification model loaded on GPU: {path}")
        return tok, model, id2label
    except Exception as e:
        print(f"   Failed to load token classification model {path}: {e}")
        return None, None, None
...
INTENT_EXAMPLES = {
    ...
    "view_cart": [
        ...
        "show my cart", "view cart", "view my cart",
        "see my cart", "see cart", "open cart", "open my cart",
        "what's in my cart", "whats in my cart", "cart me kya hai",
        "mera cart dikhao", "cart dekho", "apna cart dikhao",
        "cart summary", "check my cart", "maine kya add kiya",
        "show my basket", "meri basket", "view basket",
        "what did I add", "cart kholo", "cart details",
        "kitne items hai cart me", "cart me kitna hai",
    ],
}
...
def _compute_embedding(text: str) -> Optional[torch.Tensor]:
    ...
    # Mean pooling of last hidden state (keep on same device)
...
async def classify(req: ParseReq):
    try:
        ...
        if encoder_model and encoder_tokenizer:
            ...
            result["embedding"] = pooled[0].cpu().tolist()
        ...
        if tone_model and tone_tok:
            ...
            with torch.no_grad():
                out = tone_model(**x)
            conf, idx = softmax_top(out.logits[0].cpu())
