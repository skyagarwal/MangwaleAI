...
export class ExternalVendorService {
  ...
  constructor(
    ...
    this.phpBackendUrl = this.configService.get<string>('PHP_BACKEND_URL') || 
                         ...
                         this.configService.get<string>('php.baseUrl');
    
    if (!this.phpBackendUrl) {
      this.logger.warn('⚠️ PHP_BACKEND_URL not configured - external vendor search may not work');
    }
    
    this.googleApiKey = this.configService.get<string>('GOOGLE_MAPS_API_KEY') || '';
    
    this.logger.log('✅ ExternalVendorService initialized');
    this.logger.log(`   PHP Backend: ${this.phpBackendUrl || 'NOT CONFIGURED'}`);
  }
