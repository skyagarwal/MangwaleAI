...
export class PersonalizedSearchService {
  ...
  constructor(
    ...
    this.logger.log(`✅ PersonalizedSearchService initialized (Search API: ${this.searchApiUrl})`);
  }
  ...
  async search(options: PersonalizedSearchOptions): Promise<PersonalizedSearchResult> {
    ...
    // Execute search with V3 Hybrid (BM25 + KNN)
    try {
      const endpoint = module === 'food' 
        ? `${this.searchApiUrl}/search/hybrid/food`
        : module === 'ecom'
        ? `${this.searchApiUrl}/search/hybrid/ecom`
