...
export class ConversationLoggerService implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Get the PostgreSQL connection pool for direct queries
   * Used by other services that need to query the database
   */
  getPool(): Pool | null {
  ...
  /**
   * Get logs with intent data for analytics
   */
  async getLogsWithIntent(startDate: Date, endDate: Date): Promise<Array<{
    intent: string;
    confidence: number;
    timestamp: Date;
  }>> {
    try {
      const result = await this.pool.query(`
        SELECT 
          variables->'nlu'->>'intent' as intent,
          (variables->'nlu'->>'confidence')::float as confidence,
          created_at as timestamp
        FROM conversation_messages
        WHERE message_type = 'user'
          AND created_at >= $1
      ...
      this.logger.error(`❌ Failed to get logs with intent: ${error.message}`);
      return [];
    }
  }
}
