...
export class OrderLearningService {
  ...
  // In-memory cache for user preferences with TTL-based eviction
  private userPreferencesCache: Map<string, { data: UserPreferences; expiresAt: number }> = new Map();
  private readonly PREF_CACHE_TTL = 30 * 60 * 1000; // 30 minutes
  
  // Session interactions for learning with TTL-based eviction
  private sessionInteractions: Map<string, { events: UserInteractionEvent[]; expiresAt: number }> = new Map();
  private readonly SESSION_CACHE_TTL = 60 * 60 * 1000; // 1 hour
  ...
  constructor(
    ...
    this.cleanupInterval = setInterval(() => this.cleanupExpiredEntries(), 10 * 60 * 1000);
  }
  ...
  private cleanupExpiredEntries(): void {
    ...
    for (const [key, entry] of this.userPreferencesCache) {
      if (now > entry.expiresAt) {
        this.userPreferencesCache.delete(key);
        prefsCleaned++;
      }
    }
    for (const [key, entry] of this.sessionInteractions) {
      if (now > entry.expiresAt) {
        this.sessionInteractions.delete(key);
        sessionsCleaned++;
      }
    }
    if (prefsCleaned + sessionsCleaned > 0) {
      this.logger.debug(`🧹 Cleaned ${prefsCleaned} prefs, ${sessionsCleaned} sessions from cache`);
    }
  }
  ...
  async trackInteraction(event: UserInteractionEvent): Promise<void> {
    try {
      // Store in session
      const cached = this.sessionInteractions.get(event.sessionId);
      const sessionEvents = cached?.events || [];
      sessionEvents.push(event);
      this.sessionInteractions.set(event.sessionId, { events: sessionEvents, expiresAt: Date.now() + this.SESSION_CACHE_TTL });
  ...
  private async updateUserPreferences(event: UserInteractionEvent): Promise<void> {
    ...
    this.userPreferencesCache.set(event.userId, { data: prefs, expiresAt: Date.now() + this.PREF_CACHE_TTL });
  }
  ...
  async getUserPreferences(userId: string): Promise<UserPreferences> {
    ...
    if (cached) {
      this.userPreferencesCache.delete(userId); // Expired
    }

    // Build from order history
    const prefs = await this.buildUserPreferences(userId);
    this.userPreferencesCache.set(userId, { data: prefs, expiresAt: Date.now() + this.PREF_CACHE_TTL });
    return prefs;
  }
  ...
  async learnFromSession(sessionId: string): Promise<void> {
    const cached = this.sessionInteractions.get(sessionId);
  ...
  async suggestImprovements(): Promise<string[]> {
    ...
    for (const entry of this.sessionInteractions.values()) {
