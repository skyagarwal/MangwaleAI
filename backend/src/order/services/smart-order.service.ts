...
export class SmartOrderService {
  ...
  constructor(
    ...
    this.phpApiUrl = this.configService.get('PHP_API_URL');
  }
  ...
  /**
   * Search for restaurant/store in Mangwale partner network using OpenSearch V3 Hybrid
  ...
  async searchMangwaleNetwork(
    ...
    try {
      // Primary: Search via V3 Hybrid API (BM25 + KNN)
      const searchParams: Record<string, any> = {
        q: restaurantName,
        type: 'stores', // Search stores not items
      ...
      if (userLocation) {
        ...
        searchParams.lon = userLocation.lng;
      }

      const response = await firstValueFrom(
        this.httpService.get(
          `${this.configService.get('SEARCH_API_URL')}/search/hybrid/food`,
          { params: searchParams, timeout: 3000 }
        )
      );
      
      // Parse stores from V3 response (V3 returns stores array for type=stores)
      const stores = response.data?.stores || response.data?.items || response.data?.results || [];
      const restaurants: SearchResult[] = stores.map((store: any) => ({
      ...
      this.logger.log(`V3 Hybrid found ${restaurants.length} restaurants for "${restaurantName}"`);
      ...
      this.logger.warn(`V3 Hybrid search failed, falling back to PHP API: ${error.message}`);
