...
export class VoiceWebhookController {
  ...
  async processSpeech(
    ...
    try {
      ...
      // 🎯 UNIFIED ARCHITECTURE: Process through MessageGateway
      const language = await this.sessionService.getData(parsed.callSid, 'language') || 'en-IN';
      const result = await this.messageGateway.handleVoiceMessage(parsed.callSid, speechText, {
      ...
      if (result?.success) {
        // Get response from session (set by ContextRouter)
        const responseText = await this.sessionService.getData(parsed.callSid, 'last_bot_response') || 'I understand. How can I help you further?';
