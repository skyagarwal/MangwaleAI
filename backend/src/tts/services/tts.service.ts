    ...
    try {
      ...
      if (!provider || provider === 'auto') {
        try {
          ...
          this.logger.warn(`Failed to query TTS models from DB: ${dbError.message}`);
      ...
      try {
        await this.logCollector.logTtsError('All TTS providers failed', error.stack, {
          message: error.message,
          provider: dto.provider,
          language: dto.language,
          voice: dto.voice,
          textChars: dto.text ? dto.text.length : 0,
        });
      } catch {
