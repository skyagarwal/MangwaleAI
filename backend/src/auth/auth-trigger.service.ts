...
export class AuthTriggerService {
  ...
  requiresAuth(action: string, module: string): boolean {
    // Philosophy: Guest-first approach
    // - Users can browse, search, and add to cart without logging in
    // - Auth is required only at checkout/payment stage
    const authRequiredActions: Record<string, string[]> = {
      food: ['checkout', 'apply_coupon'], // Removed place_order, add_to_cart - allow guest browsing
      parcel: ['book_delivery', 'create_order', 'confirm_booking'],
      ecom: ['checkout', 'buy_now'], // Removed add_to_cart - allow guest browsing
  ...
  getAuthPrompt(action: string, module: string): string {
    const prompts: Record<string, string> = {
      food: "To confirm your order, please login or signup to continue.",
      parcel: "To book a Parcel Delivery, please login or signup to continue.",
      ecom: "To proceed to checkout, please login or signup to continue.",
      tracking: "To track your orders, please login or signup to continue.",
      games: "To claim your rewards, please login or signup to continue.",
      complaints: "To file a complaint, please login or signup to continue.",
      profile: "To view your profile, please login or signup to continue.",
      wallet: "For wallet access, please login or signup to continue.",
    };

    const prompt = prompts[module] || "Please login or signup to continue.";
  ...
  getAuthExplanation(action: string, module: string): string {
    const explanations: Record<string, string> = {
      ...
      claim_reward: "Login is required to process the reward transfer.",
      book_delivery: "Login is required for the delivery partner to contact you and track your parcel.",
