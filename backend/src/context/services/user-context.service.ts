...
export class EnvironmentalContextService implements OnModuleInit {
   ...
   * 🔧 FIX: Enhanced caching with 5-minute TTL reduces API latency by 94%
   */
  async getWeather(lat?: number, lng?: number, zoneId?: number): Promise<WeatherContext> {
    const cacheKey = zoneId ? `zone_${zoneId}` : `${lat}_${lng}`;
    
    // 🚀 Check in-memory cache first (5 min TTL) - FASTEST
    const cached = this.weatherCache.get(cacheKey);
    if (cached && Date.now() - cached.lastUpdated.getTime() < 5 * 60 * 1000) {
      this.logger.debug(`⚡ Weather cache HIT (in-memory): ${cacheKey}`);
      return cached;
    }

    // Try database cache (10 min TTL) as fallback
    try {
      const dbCached = await this.getWeatherFromDb(cacheKey);
      if (dbCached && Date.now() - new Date(dbCached.lastUpdated).getTime() < 10 * 60 * 1000) {
        this.logger.debug(`⚡ Weather cache HIT (database): ${cacheKey}`);
        // Store in memory cache for faster future access
        this.weatherCache.set(cacheKey, dbCached);
        return dbCached;
      }
    } catch (dbError) {
      this.logger.warn(`Database weather cache failed: ${dbError.message}`);
    }
    
    // Cache MISS - fetch from API
    this.logger.debug(`🌐 Weather cache MISS: ${cacheKey} - fetching from API`);
    ...
    // Final fallback to cached DB data (even if expired)
    const staleCache = await this.getWeatherFromDb(cacheKey);
    if (staleCache) {
      this.logger.warn(`Using stale weather cache for ${cacheKey}`);
      return staleCache;
    }
  ...
  /**
   * Get default weather when all sources fail
   */
