#!/usr/bin/env python3
"""
IndicBERT v3 Fine-tuning Script for Mangwale NLU
=================================================

Fine-tunes IndicBERT-v3 (ai4bharat/IndicBERT-v3-270M) for intent classification.
Architecture: Gemma 3-based with bidirectional attention, 270M params.

Optimized for RTX 3060 12GB:
- Gradient checkpointing to reduce memory
- BF16 mixed precision
- Batch size 4 with gradient accumulation
- Max length 64 (intents are short)

Training Data: nlu_final_v4_with_missing_intents.jsonl (3,938 examples, 50 intents)
"""
...
# Base model - IndicBERT v3 (Gemma 3 architecture)
BASE_MODEL = os.environ.get('BASE_MODEL', 'ai4bharat/IndicBERT-v3-270M')
# Fallback to local copy if downloaded
...
OUTPUT_DIR = os.environ.get(
    'OUTPUT_DIR',
    '/home/ubuntu/nlu-training/models/indicbert_v3_finetuned'
)
...
BATCH_SIZE = int(os.environ.get('BATCH_SIZE', '4'))  # Small batch to fit in memory
GRAD_ACCUM = int(os.environ.get('GRAD_ACCUM', '8'))  # Effective batch = 4 * 8 = 32
NUM_EPOCHS = int(os.environ.get('NUM_EPOCHS', '8'))
LEARNING_RATE = float(os.environ.get('LEARNING_RATE', '2e-5'))
MAX_LENGTH = int(os.environ.get('MAX_LENGTH', '64'))  # Short intents
WARMUP_RATIO = 0.1
WEIGHT_DECAY = 0.01
...
def main():
    ...
    # ---- Load training data ----
    logger.info(f'\n[1/7] Loading training data from {TRAINING_DATA}')
    data = load_training_data(TRAINING_DATA)
    logger.info(f'Loaded {len(data)} examples')

    # Build label mapping
    intent_counts = Counter(d['intent'] for d in data)
    # Filter intents with < 3 examples (not enough for stratified split)
    ...
    logger.info(f'Examples after filtering: {len(data)}')

    # Show distribution
    for intent, count in sorted(intent_counts.items(), key=lambda x: -x[1])[:10]:
        logger.info(f'  {intent}: {count}')
    logger.info(f'  ... and {len(all_intents) - 10} more')
    ...
    # ---- Load model ----
    logger.info('\n[3/7] Loading IndicBERT v3 model and tokenizer')
    ...
    # Save in float32 for inference compatibility
    model_to_save = trainer.model
    model_to_save = model_to_save.float()
