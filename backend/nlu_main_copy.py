...
HINDI_NORMALIZATIONS = {
    ...
    r'\bpickp\b': 'pickup',
    # Delivery patterns - add food context
    r'\bghar pe bhej\b': 'ghar pe khana bhej',
    r'\bjaldi bhej\b': 'jaldi khana bhej',
}

def normalize_hindi_text(text: str) -> str:
    """Normalize common Hindi misspellings and add food context."""
...
async def classify(req: ParseReq):
    try:
        result = {}
        # Embedding (for downstream) if encoder is available
        ...
        # Intent classification - HYBRID: trained model + embedding fallback
        ...
        if trained_intent and trained_conf >= TRAINED_THRESHOLD:
            # Trained model is confident enough
            result.update({"intent": trained_intent, "intent_conf": float(trained_conf), "method": "trained"})
        elif embedding_intent and embedding_conf >= EMBEDDING_THRESHOLD:
            # Embedding fallback is confident
            result.update({"intent": embedding_intent, "intent_conf": float(embedding_conf), "method": "embedding"})
        elif trained_intent and trained_conf > embedding_conf:
            # Trained model is best available even if low confidence
            result.update({"intent": trained_intent, "intent_conf": float(trained_conf), "method": "trained-low"})
        elif embedding_intent:
            # Embedding is best available
            result.update({"intent": embedding_intent, "intent_conf": float(embedding_conf), "method": "embedding-low"})
        else:
            result.update({"intent": "default", "intent_conf": 0.0, "method": "none"})
