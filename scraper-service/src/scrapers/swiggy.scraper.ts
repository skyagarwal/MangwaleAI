...
export class SwiggyScraper {
  ...
  /**
   * Parse Swiggy preloaded state JSON from HTML
   */
  private parsePreloadedState(html: string): any | null {
    const $ = cheerio.load(html);
    let raw = '';
    $('script').each((_, el) => {
      const txt = $(el).html() || '';
      if (txt.includes('__PRELOADED_STATE__')) {
        raw = txt;
      }
    });
    if (!raw) return null;
    const match = raw.match(/__PRELOADED_STATE__\s*=\s*(\{[\s\S]*?\});?/);
  ...
  async searchRestaurant(
    ...
    try {
      ...
      // Wait briefly and parse preloaded state if available
      await this.delay(2000);
      const html = await page.content();
      let restaurants: SwiggyRestaurant[] = [];
      const state = this.parsePreloadedState(html);
      if (state) {
        const cards = state?.pages?.search?.cards || state?.cards || [];
        for (const card of cards) {
          const data = card?.card?.card || card?.data || {};
          const grid = data?.gridElements?.infoWithStyle?.restaurants || [];
          for (const r of grid) {
            const info = r?.info || {};
            const url = info?.cta?.link ? `https://www.swiggy.com${info.cta.link}` : '';
            restaurants.push({
              id: String(info?.id || '').trim(),
              name: info?.name || '',
              url,
              rating: parseFloat(info?.avgRating || info?.avgRatingString || '0') || 0,
              ratingCount: info?.totalRatingsString || '',
              cuisine: info?.cuisines || [],
              area: info?.areaName || '',
              deliveryTime: info?.sla?.slaString || '',
              costForTwo: parseInt(String(info?.costForTwo || info?.costForTwoMessage).replace(/[^0-9]/g, ''), 10) || 0,
              isPromoted: !!info?.promoted,
              offers: [],
              confidence: this.calculateSimilarity(name, info?.name || '')
            });
          }
        }
      }
  ...
  async scrapeRestaurant(url: string): Promise<{
    ...
    try {
      ...
      const html = await page.content();
      const state = this.parsePreloadedState(html);
      let restaurant: SwiggyRestaurant;
      if (state) {
        const info = state?.pages?.menu?.restaurant?.info || state?.restaurant?.info || {};
        restaurant = {
          id: String(info?.id || url.split('/').pop() || '').trim(),
          name: info?.name || '',
          url,
          rating: parseFloat(info?.avgRating || info?.avgRatingString || '0') || 0,
          ratingCount: info?.totalRatingsString || '',
      ...
      // Parse menu items via state if available, else DOM fallback
      const menu: SwiggyMenuItem[] = [];
      if (state) {
        this.logger.debug(`Parsing Swiggy menu from preloaded state`);
        const cards = state?.pages?.menu?.cards || state?.cards || [];
        this.logger.debug(`Found ${cards.length} cards in preloaded state`);
        for (const card of cards) {
          const group = card?.groupedCard?.cardGroupMap?.REGULAR?.cards || [];
          this.logger.debug(`Found ${group.length} group cards`);
          for (const gc of group) {
            const menuSection = gc?.card?.card?.itemCards || [];
            const title = gc?.card?.card?.title || '';
            this.logger.debug(`Section "${title}" has ${menuSection.length} items`);
            for (const ic of menuSection) {
              const info = ic?.card?.info || {};
              if (info?.name) {
                menu.push({
                  id: String(info?.id || ''),
                  name: info?.name || '',
                  price: parseInt(String(info?.price || info?.defaultPrice || '0').replace(/[^0-9]/g, ''), 10) / 100 || 0,
                  description: info?.description || '',
                  category: title || info?.category || '',
                  isVeg: info?.isVeg === 1 || info?.isVeg === true,
                  isBestseller: !!info?.isBestseller,
                  inStock: info?.inStock !== 0,
                  ratings: {
                    rating: parseFloat(info?.ratings?.aggregatedRating?.rating || '0') || 0,
        ...
        this.logger.info(`Parsed ${menu.length} items from Swiggy preloaded state`);
      }
      if (menu.length === 0) {
        const $ = cheerio.load(html);
        let currentCategory = '';
