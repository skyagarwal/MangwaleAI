 ...
 * 
 * Usage: node scripts/nlu-search-audit.js
 */
...
function httpPost(url, data) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.error(`Parse error for: ${data.text} - Response: ${body.substring(0, 200)}`);
    ...
    req.on('error', (err) => {
      console.error(`Request error for: ${data.text} - ${err.message}`);
      reject(err);
    });
    req.on('timeout', () => {
      req.destroy();
...
function addTestCase(category, query, expected) {
...
for (let i = 0; i < 50; i++) {
  const category = random(Object.keys(FOOD_ITEMS));
...
recommendVariants.forEach(query => {
  addTestCase('recommendation', query, {
...
for (let i = 0; i < 20; i++) {
  ...
  const allItems = [...FOOD_ITEMS.northIndian, ...FOOD_ITEMS.southIndian];
  const item = random(allItems);
...
for (let i = 0; i < 20; i++) {
  const allItems = [...FOOD_ITEMS.fastFood, ...FOOD_ITEMS.streetFood];
  const item = random(allItems);
...
async function runTests() {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    while (retries > 0 && !data) {
      try {
        data = await httpPost(API_URL, {
          text: tc.query,
          sessionId: `audit-${tc.id}`
        });
        
        // Validate response has required fields
        if (!data || typeof data.intent === 'undefined') {
          console.error(`Empty/invalid response for "${tc.query.substring(0, 30)}..." - retrying`);
          data = null;
          retries--;
        ...
        console.error(`Error for "${tc.query.substring(0, 30)}...": ${error.message}`);
        retries--;
        await new Promise(r => setTimeout(r, 500));
      }
    }
    ...
    if (!data || typeof data.intent === 'undefined') {
      ...
      results.push({
        ...
        passed: false,
        issues: ['API returned empty or invalid response after retries']
      });
      continue;
    }
    ...
    const issues = [];
      ...
      if (tc.expected.intent && data.intent !== tc.expected.intent) {
        ...
        const intentMap = {
          ...
          'search_product': ['search_product', 'order_food'],
          'ask_recommendation': ['ask_recommendation', 'search_product'],
          'manage_order': ['manage_order', 'modify_order', 'cancel_order']
        };
      ...
      if (tc.expected.food_items && tc.expected.food_items.length > 0) {
        ...
        for (const expected of tc.expected.food_items) {
          const found = actualFoodLower.some(f => 
    ...
    // Delay to avoid overwhelming the server (longer delay for LLM fallback scenarios)
...
function generateSummary(results, totalTime) {
  ...
  const avgTime = validResults.length > 0 
    ? validResults.reduce((sum, r) => sum + r.actual.processingTimeMs, 0) / validResults.length
    : 0;
  ...
  Object.entries(byCategory).forEach(([category, catResults]) => {
    ...
    if (failedCases.length > 0) {
      ...
      failedCases.forEach(r => {
        ...
        const actual = `${r.actual.intent} (${(r.actual.confidence || 0).toFixed(2)})`;
