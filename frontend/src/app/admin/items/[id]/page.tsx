...
export default function EditItemPage() {
  ...
  const { loading: loadingItem, error: itemError, getItem, updateItem, clearError } = useAdminItems();
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setFormData({
        ...
        module_id: item.module_id || 4,
        image: item.image || '',
      });

      // Load stores and categories for the same module
      const [storesData, categoriesData] = await Promise.all([
        fetchStores({ module_id: item.module_id, limit: 500, status: 1 }),
        fetchCategories({ module_id: item.module_id, limit: 500, status: 1 }),
      ]);
  ...
  }, [itemId, getItem, fetchStores, fetchCategories]);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-blue-100 mt-2">
              Update item details and sync to search
            </p>
          </div>
        </div>
      </div>
